        ...
        foreach (Entity entity in Entities)
        {
            if (entity != null)
            {
                if (entity.index >= 3)
                {
                    ...
                    if (entity.character.Id == DataManager.EntityId.MerakMage || (Random.Range(0,2) == 1 &&((entity.character.Id == DataManager.EntityId.Connor && Entities[0] != null && Entities[1] != null) || entity.character.Id == DataManager.EntityId.CrystalizedDragon)))
                    {
    ...
    public void SpawnItem()
    {
        ...
        switch (DataManager.CurrentLevelPackage.backroundId)
        {
            case DataManager.BackroundId.Forest:
                ...
                break;
            case DataManager.BackroundId.Minegate:
                ItemIdToSpawn = Random.Range(1, 5);
                break;
            default:
                ItemIdToSpawn = Random.Range(1, 5);
